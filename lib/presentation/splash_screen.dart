...
class _SplashScreenState extends State<SplashScreen> {
 ...
 Future<void> _handleNonVehicleDriverRouting(String token) async {
    try {
      ...
      final verificationStatus = await _checkNonVehicleVerificationStatus(token);
      ...
      if (verificationStatus == 'rejected' || verificationStatus == 'declined') {
        print('❌ Verification rejected - Showing error');
        Get.offAllNamed('/verification-pending'); // or create a rejection screen
      ...
      if (verificationStatus == 'approved' || 
        ...
        final subscriptionController = Get.find<NonVehicleSubscriptionController>();
        await subscriptionController.loadSubscriptionStatus();

        final subStatus = subscriptionController.subscriptionStatus.value.toLowerCase();
        print('💳 Subscription Status: $subStatus');
        print('═══════════════════════════════════════');

        // ✅ CHECK: Active subscription
        if (subStatus == 'active' || subStatus == 'subscribed' || subStatus == true) {
        ...
        print('❌ No active subscription - Going to NON-VEHICLE SUBSCRIPTION SCREEN');
 ...
 Future<String> _checkNonVehicleVerificationStatus(String token) async {
  try {
    ...
    final response = await http.get(
      Uri.parse('https://backend.ridealmobility.com/api/non-vehicle-driver/profile/$driverId'),
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      else if (data['driver'] != null && data['driver']['verificationStatus'] != null) {
        status = data['driver']['verificationStatus'].toString().toLowerCase();
  ...
  Future<void> _handleDriverRouting(String token) async {
    try {
      ...
      print('👤 [SPLASH] Profile loaded for: ${profileController.driverProfile.value?.name}');
      ...
      if (profileController.driverProfile.value?.verification != null) {
        final profileKycStatus = profileController.driverProfile.value?.verification?.status?.toLowerCase();
      ...
      if (kycStatus == 'not_submitted' || kycStatus == 'notsubmitted' || kycStatus == 'unknown') {
      ...
      if (kycStatus == 'approved' || kycStatus == 'accepted' || kycStatus == 'verified') {
        print('✅ [SPLASH] KYC status is OK (${kycStatus}) - Checking subscription...');
        print('═══════════════════════════════════════');
        ...
        final hasActiveSubscription = subscriptionController.hasSubscription.value;
        print('💳 [SPLASH] Has Active Subscription: $hasActiveSubscription');
        print('═══════════════════════════════════════');

        if (hasActiveSubscription) {
          print('✅ [SPLASH] Active subscription found - Preparing dashboard...');
        ...
        print('❌ [SPLASH] No active subscription - Navigating to SUBSCRIPTION SCREEN');
        Get.offAllNamed(Routes.SUBSCRIPTION);
        return;
      }

      // Fallback
      print('⚠️ [SPLASH] Fallback hit - No specific status matched. Going to KYC page as safety measure.');
      Get.offAllNamed('/kyc-documents');
    } catch (e, stack) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true && data['data'] != null) {
          final status = (data['data']['status'] ?? data['status'])?.toString().toLowerCase();
