...
import '../services/non_vehicle_payout_service.dart';
import '../core/utils/app_snackbar.dart';

class NonVehiclePayoutController extends GetxController {
  ...
  var selectedPayoutMethod = 'UPI'.obs;

  // Payout methods
  final List<String> payoutMethods = ['UPI', 'BANK'];
  ...
  Future<void> fetchPayoutHistory() async {
    try {
      ...
      final response = await NonVehiclePayoutService.getMyPayouts();
  ...
  // In NonVehiclePayoutController
  Future<void> requestWithdrawal() async {
    print('🚀 requestWithdrawal called');
    ...
    try {
      ...
      if (amount == null) {
        ...
        showErrorSnackBar('Invalid amount', title: 'Error');
        return;
      }

      print('📤 Calling NonVehiclePayoutService.requestPayout...');
      ...
      if (selectedPayoutMethod.value == 'UPI') {
        ...
        print('🏦 Account: $accountNumber, IFSC: $ifscCode');
      }

      // ⭐ CALL SERVICE WITH ALL PARAMETERS
      final result = await NonVehiclePayoutService.requestPayout(
        amount: amount,
        payoutMethod: selectedPayoutMethod.value, // ⭐ BANK or UPI
  ...
  Future<void> verifyOTPAndCompleteWithdrawal() async {
    ...
    try {
      ...
      print('   _pendingUpiId: $_pendingUpiId');
      ...
      final response = await NonVehiclePayoutService.verifyPayoutOTP(
  ...
  bool _validateForm() {
    ...
    if (amount < 270) {
      print('   ❌ Amount is less than minimum: $amount');
      showWarningSnackBar(
        'Minimum withdrawal amount is ₹270',
        title: 'Validation Error',
      );
      return false;
    }
